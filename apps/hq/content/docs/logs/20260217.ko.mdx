...
## 마이그레이션 종합 검증 실행 (Task 23)

**범위**: `apps/web`, 루트 테스트 러너

### 변경 사항

| 파일 | 변경 사항 |
| --- | --- |
| `.sisyphus/evidence/task-23-typecheck.txt` | `apps/web`에서 실행한 `bun run type-check` 결과를 저장. |
| `.sisyphus/evidence/task-23-build.txt` | `apps/web`에서 실행한 `bun run build` 결과를 저장. |
| `.sisyphus/evidence/task-23-tests.txt` | 루트 `bun test` 실행 결과와 실패 상세를 저장. |

### 기술적 결정

1. **요구된 실행 경계 준수**: 타입체크/빌드는 `apps/web`, 테스트는 루트에서 실행해 검증 범위를 고정했다.
2. **증거 우선 실행**: 각 명령의 stdout/stderr를 즉시 task 전용 증거 파일로 기록해 추적성을 확보했다.
3. **실패 은닉 금지**: `bun test` 실패 출력을 그대로 보존하고 Playwright `test.describe()` 비정상 호출 오류를 명시했다.

### 검증

- TypeScript: 통과 (`apps/web && bun run type-check`)
- Build: 통과 (`apps/web && bun run build`)
- Tests: 실패 (`bun test` 루트 실행; `apps/web/e2e/core-journey.spec.ts`에서 Playwright unhandled error 발생)

### 다음 단계

1. Playwright E2E는 `playwright test`로 분리 실행하거나 Bun 테스트 탐색 대상에서 E2E 파일을 제외해 러너 충돌을 제거.
2. 러너 분리 후 `bun test`를 재실행하고 `.sisyphus/evidence/task-23-tests.txt`를 갱신.
