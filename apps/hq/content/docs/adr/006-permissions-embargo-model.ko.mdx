...
- **파일 연동:** Block ACL 변경 시 그에 따라 파일 권한 업데이트 호출 ([ADR 5](/adr/005-file-storage-access-control))
...
<Cards>
  <Card title="그룹 공유" href="/adr/007-presence-cursors">
    많은 사람에게 공유를 단순화하기 위해 그룹/팀 개념 추가. 30명 개별 대신
    "Research Team"에 공유.
  </Card>
  <Card title="역할 & 권한 매트릭스" href="/adr/007-presence-cursors">
    역할 공식화: "viewer" vs "editor" vs "commenter". 권한 타입당 ACL 구조 확장.
  </Card>
  <Card title="계층적 권한" href="/adr/007-presence-cursors">
    Block의 플래그: "오버라이드 안 되면 부모로부터 ACL 상속". 명시적 ACL을 찾을
    때까지 위로 걸어감.
  </Card>
  <Card title="임시 접근 링크" href="/adr/005-file-storage-access-control">
