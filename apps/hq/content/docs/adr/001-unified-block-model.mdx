...
- (Optionally) structural fields: e.g. `parentId` or an order index if we store hierarchy directly. However, hierarchical or semantic relationships are primarily managed by the separate Edges model (see [ADR 4](/adr/004-edge-types-dialectical-graph))
...
>
  ...
  <Tab value="Graph Database">
    **Graph Database or RDF Triple Store:** Using a dedicated graph DB or RDF
    ...
    4](/adr/004-edge-types-dialectical-graph)) which is conceptually similar to
    the typical SQL approach of nodes and an edges table without needing a
    separate DB.
  </Tab>
</Tabs>
...
<Cards>
  ...
  <Card title="Federation of Blocks" href="/adr/003-collaboration-protocol">
