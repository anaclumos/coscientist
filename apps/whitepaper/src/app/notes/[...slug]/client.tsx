...
function NotesContent({ rootSlug, allNotes, initialNotesData }: NotesPageClientProps) {
  const { stack, focusIndex, pushNote, popNote, focusPane, setStack } = useNoteStack(rootSlug);
  const [keyboardFocusIndex, setKeyboardFocusIndex] = useState(() => Math.max(0, initialNotesData.length - 1));
  const [dragState, setDragState] = useState<{ index: number; startX: number } | null>(null);
  ...
  const handleLinkClick = useCallback((slug: string, fromPaneIndex: number) => {
    pushNote(slug, fromPaneIndex);
  }, [pushNote]);
  
  const handleExpandPane = useCallback((index: number) => {
  ...
  const handleAllNotesClick = useCallback((slug: string) => {
    ...
    return {
      ...
      onPointerUp: (e: React.PointerEvent) => {
        ...
        if (Math.abs(deltaX) > threshold) {
          ...
          const newIndex = Math.max(1, Math.min(stack.length - 1, index + direction));
          if (newIndex !== index) {
