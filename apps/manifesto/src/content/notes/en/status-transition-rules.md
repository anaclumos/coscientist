...
Key transitions include: tentative → contested  is registered), contested → conditionally supported  or clarifying [assumptions](./assumption)), conditionally supported → robust , robust → refuted , and any state → obsolete  severs the claim's meaning).
