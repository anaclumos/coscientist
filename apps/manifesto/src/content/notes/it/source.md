...
Non tutte le fonti sono uguali. Una fonte primaria riporta osservazioni originali; una fonte secondaria riassume o interpreta; una fonte terziaria compila a partire da fonti secondarie. Ogni passaggio introduce un potenziale scostamento, e [Coscienziato](./coscientist) tiene traccia di queste distinzioni tramite archi `cites` espliciti nel [Grafo Dialettico](./dialectical-graph).

Le fonti sono nodi nel grafo ) con metadati come autore, data e contesto. Quando un’affermazione viene contestata, la fonte fornisce il punto di partenza per una nuova verifica.
